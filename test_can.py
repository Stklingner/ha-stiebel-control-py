"""
Tests for the new CAN interface implementation with separation of concerns.
...
class TestCanLayers(unittest.TestCase):
    ...
    def test_protocol_layer(self):
        ...
        self.assertIn(mock_handler, protocol.signal_handlers)
        
        # Test removing a signal handler
        protocol.remove_signal_handler(mock_handler)
        self.assertNotIn(mock_handler, protocol.signal_handlers)
    ...
    def test_signal_handler(self):
        ...
        self.assertIn(mock_callback, handler.signal_callbacks[(0x180, "TEST_SIGNAL")])
        
        handler.remove_signal_callback("TEST_SIGNAL", 0x180, mock_callback)
        self.assertFalse(any(mock_callback in callbacks for callbacks in handler.signal_callbacks.values()))
        
        # Test global callbacks
        handler.add_global_callback(mock_callback)
        self.assertIn(mock_callback, handler.global_callbacks)
        
        handler.remove_global_callback(mock_callback)
        self.assertNotIn(mock_callback, handler.global_callbacks)
        ...
        self.assertEqual(handler.latest_values[(0x180, "TEST_SIGNAL")], 42.5)
...
def main():
    ...
    try:
        ...
        logger.info("All CAN interface tests passed")
        return 0
